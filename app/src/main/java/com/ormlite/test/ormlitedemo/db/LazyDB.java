...
public class LazyDB {
    ...
    private volatile static LazyDB INSTANCE;

    private LazyDB() {

    }

    public static LazyDB getSingleton() {
        if (INSTANCE == null) {
            synchronized (LazyDB.class) {
                if (INSTANCE == null) {
                    INSTANCE = new LazyDB();
                }
            }
     ...
     */
    public void initLazyDB(Context context, int daoMaxSize) {
     ...
     * @param daoMaxSize dao的最大缓存数量(小于等于0则使用默认,默认为10)
     */
    public void initLazyDB(Context context, DBParam dbParam, int daoMaxSize) {
        if (dbParam.isSaveOnSDCard()) {
            String dbPath = dbParam.getTargetDirectory();
            if (!TextUtils.isEmpty(dbPath)) {
                ...
                dbHelper = new LazyDBHelper(context, dbParam.getDbName(), null,
                        dbParam.getDbVersion(), dbFile, daoMaxSize);
            } else {
                dbHelper = new LazyDBHelper(context, dbParam.getDbName(), null,
                        dbParam.getDbVersion(), daoMaxSize);
            }
     ...
     * @return
     * @throws SQLException
     * @throws NullPointerException Dao<T,ID>
     * @throws
     * @see [类、类#方法、类#成员]
     */
     ...
     * @return
     * @throws SQLException
     * @throws NullPointerException QueryBuilder<T,?>
     * @throws
     * @see [类、类#方法、类#成员]
     */
     ...
     * @return
     * @throws SQLException
     * @throws NullPointerException DeleteBuilder<T,?>
     * @throws
     * @see [类、类#方法、类#成员]
     */
     ...
     * @return
     */
    public <T> int batchInsert(List<T> t) {
        if(t == null || t.isEmpty()){
            return -1;
        }
        try {
            return dbHelper.batchInsert(t);
        } catch (Exception e) {
            Log.e(TAG, "批量插入失败");
        }
        return -1;
